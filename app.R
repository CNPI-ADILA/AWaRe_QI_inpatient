...
server <- function(input, output, session) {
  ...
  output$download_combined <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Apply Treatment standardization for surgical prophylaxis before QI eligibility check
      data_with_eligibility <- data_patients_all %>%
        mutate(
          Treatment_standardized = ifelse(
            Indication %in% c("SP1", "SP2", "SP3") & (is.na(Treatment) | Treatment == ""),
            "EMPIRICAL",
            Treatment
          ),
          AWaRe_compatible = case_when(
            startsWith(`Diagnosis code`, "Proph") &
              ...
              `Age years` >= 18 &
              Treatment_standardized == "EMPIRICAL" ~ TRUE,
            !startsWith(`Diagnosis code`, "Proph") &
        ...
        select(-AWaRe_compatible, -Treatment_standardized)
